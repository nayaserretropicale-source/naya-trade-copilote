...
export async function POST(req: NextRequest) {
  try {
    ...
    const { symbol, amountXof } = await req.json();
    ...
    await supabaseAdmin.from("transactions").insert({ portfolio_id: portfolio.id, symbol: sym, side: "buy", quantity, price, total: amountUsd, source: "manual" });
