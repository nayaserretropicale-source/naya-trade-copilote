...
export async function generateSuggestions(portfolioId: string) {
  ...
  const context = {
    ...
    mes_positions: posDetail.map((p) => ({ symbol: p.symbol, part_du_portefeuille_pct: total > 0 ? Number((p.value / total * 100).toFixed(0)) : 0, gain_perte_pct: Number(p.plPct.toFixed(0)), valeur_fcfa: Math.round(p.value * rate) })),
...
Pour les VENTES (action "sell"), UNIQUEMENT pour des raisons de prudence, jamais de prediction:
- Si une position depasse ~35% du portefeuille, suggere d'ALLEGER pour reequilibrer.
- Si une position Dynamique/volatile a beaucoup monte ET pese lourd, suggere de prendre une PARTIE des gains.
- Donne "fraction": 0.5 (alleger) ou 1 (tout vendre). Prefere 0.5, sauf concentration extreme.
- Ne suggere jamais de vendre une position raisonnable et bien diversifiee.

Si le portefeuille est deja equilibre, propose une seule "watch" ou explique qu'il vaut mieux ne rien faire. Ne force jamais.
